...
class Solution {
    public int maxFreeTime(int eventTime, int[] startTime, int[] endTime) {
        ...
        int []freeArr = new int[n+1];

        freeArr[0] = startTime[0];

        for(int i=1;i<n;i++){
            freeArr[i] = startTime[i] - endTime[i-1];
        }

        freeArr[n] = eventTime - endTime[n-1];

        int m = freeArr.length;
        int []maxRightFree = new int[m];
        int []maxLeftFree = new int[m];

        for(int i=m-2;i>=0;i--){
            maxRightFree[i] = Math.max(maxRightFree[i+1], freeArr[i+1]);
        }

        for(int i=1;i<m;i++){
            maxLeftFree[i] = Math.max(maxLeftFree[i-1], freeArr[i-1]);
        }
        ...
        for(int i=1;i<m;i++){
            ...
            if(currEventTime <= Math.max(maxLeftFree[i-1], maxRightFree[i])){
                res = Math.max(res, freeArr[i-1] + currEventTime + freeArr[i]);
            }

            res = Math.max(res, freeArr[i-1] + freeArr[i]);
        }
