...
class Solution {
    private int solve(int box, int []status, int []candies, int [][]keys, int [][]containedBoxes, Set<Integer> visited, Set<Integer> foundBoxes){
        ...
        for(int innerBox : containedBoxes[box]){
            candiesCollected += solve(innerBox, status, candies, keys, containedBoxes, visited, foundBoxes);
        }

        for(int boxKey : keys[box]){
            status[boxKey] = 1;
            if(foundBoxes.contains(boxKey)){
                candiesCollected += solve(boxKey, status, candies, keys, containedBoxes, visited, foundBoxes);
            }
        }
    ...
    public int maxCandies(int[] status, int[] candies, int[][] keys, int[][] containedBoxes, int[] initialBoxes) {
        ...
        for(int box : initialBoxes){
            candiesCollected += solve(box, status, candies, keys, containedBoxes, visited, foundBoxes);
        }
