...
class Solution {
    public int[][] merge(int[][] intervals) {
        ...
        for (int i=1;i<n;i++) {
            if (intervals[i][0] < res.get(res.size() - 1)[1]) { // Overlapping
    ...
    public boolean checkValidCuts(int n, int[][] rectangles) {
        ...
        for (int[] coord : rectangles) {
            ...
            hori.add(new int[]{x1, x2});
            vert.add(new int[]{y1, y2});
        }

        int[][] horiArr = hori.toArray(new int[hori.size()][]);
        int[][] vertArr = vert.toArray(new int[vert.size()][]);

        int[][] res1 = merge(horiArr);
        if (res1.length >= 3) {
            return true;
        }

        int[][] res2 = merge(vertArr);
