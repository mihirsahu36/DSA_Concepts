...
class Solution {
    ...
    public int solve(long i, long uniqueChars, boolean canChange){
        ...
        if(uniqueCharCount > K){
            res = 1 + solve(i + 1, 1L << characterIndex, canChange);
        }else{
            res = solve(i + 1, uniqueCharsUpdated, canChange);
        }
        ...
        if(canChange){
            for(int newCharIndex=0;newCharIndex<26;newCharIndex++){
                ...
                if(newUniqueCharCount > K){
                    res = Math.max(res, 1 + solve(i + 1, 1L << newCharIndex, false));
                }else{
                    res = Math.max(res, solve(i + 1, newSet, false));
                }
            }
        }
