...
class Solution {
    public int[] minOperations(String boxes) {
        ...
        for(int i=0;i<n;i++){ // calculate operations needed to move balls to each box from the left side
            res[i] = cumValueSum;
        ...
        for(int i=n-1;i>=0;i--){ // calculate operations needed to move balls to each box from the right side
            res[i] += cumValueSum;
            cumValue += boxes.charAt(i) == '0' ? 0 : 1;
            cumValueSum += cumValue;
        }
