...
class Solution {
    private HashMap<Integer, List<Integer>> getAdj(int [][]edges){
        ...
        for(int []edge : edges){
            ...
            adj.computeIfAbsent(v, m -> new ArrayList<>()).add(u);
        }
        return adj;
    }
    
    private void solve(int curr, HashMap<Integer, List<Integer>> adj, int parent,
    int []mark, int []count){
        if(mark[curr] == 0){
            count[0]++;
        }else{
        ...
        for(int ngbr : adj.getOrDefault(curr, new ArrayList<>())){
            if(ngbr != parent){
                ...
                solve(ngbr, adj, curr, mark, count);
            }
        }
    }
    ...
    public int[] maxTargetNodes(int[][] edges1, int[][] edges2) {
        ...
        HashMap<Integer, List<Integer>> adjA = getAdj(edges1);
        HashMap<Integer, List<Integer>> adjB = getAdj(edges2);

        int []markA = new int[n];
        Arrays.fill(markA, -1);
        markA[0] = 0;
        int []countA = new int[2];
        solve(0, adjA, -1, markA, countA);

        int []markB = new int[m];
        Arrays.fill(markB, -1);
        markB[0] = 0;
        int []countB = new int[2];
        solve(0, adjB, -1, markB, countB);

        int maxTree2 = Math.max(countB[0], countB[1]);
        int []res = new int[n];

        for(int i=0;i<n;i++){
            res[i] = (markA[i] == 0 ? countA[0] : countA[1]) + maxTree2;
        }
